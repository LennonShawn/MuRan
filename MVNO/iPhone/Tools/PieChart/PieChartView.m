...
- (id)initWithFrame:(CGRect)frame withValue:(NSMutableArray *)valueArr withColor:(NSMutableArray *)colorArr
{
    ...
    if (self) {
        ...
        UIImage *centerImage = [UIImage imageNamed:@"PieChartCenter.png"];
        ...
        self.centerView.frame = CGRectMake((frame.size.width - centerImage.size.width)/2, (frame.size.height - centerImage.size.height)/2, centerImage.size.width, centerImage.size.height);
        self.centerView.center = self.center;
        int titleWidth = 60;
        self.title = [[UILabel alloc]initWithFrame:CGRectMake((centerImage.size.width - titleWidth)/2,(centerImage.size.height - 17)/2 , titleWidth, 17)];
        self.title.backgroundColor = [UIColor clearColor];
        self.title.textAlignment = NSTextAlignmentCenter;
        self.title.font = [UIFont systemFontOfSize:14];
        self.title.textColor = [self colorFromHexRGB:@"646464"];
        self.title.text = @"";
        [self.centerView addSubview:self.title];
        
        int amountWidth = 55;
        self.amountLabel = [[UILabel alloc]initWithFrame:CGRectMake((centerImage.size.width - amountWidth), 53, amountWidth, 20)];
